...
const handleCharacterBios = () => {
  ...
  dropdown.addEventListener('change', (event) => {
    ...
    const setBio = (imgPath, name, spouse, bio, quote) => {
      ...
      cbLeft.innerHTML = ''; // Clear previous content
      cbRight.innerHTML = ''; // Clear previous content
...
async function sendMessage() {
  ...
  try {
    const response = await fetch('/.netlify/functions/chat', {  // Updated to call Netlify function
      method: 'POST',
      headers: {
